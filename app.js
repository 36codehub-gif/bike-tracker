...
// VOICE CALL FEATURE (WebRTC Audio Stream)
async function toggleVoiceCall() {
  if (!inVoiceCall) {
    try {
      ...
      // Announce Presence in Firebase
      db.ref("voice_room/" + currentRiderId).set({ active: true, time: Date.now() });

    } catch (err) {
      alert("Microphone permission deni zaroori hai voice call ke liye!");
    }
  } else {
    leaveVoiceChannel();
  }
...
function toggleTracking() {
  ...
  if (!isTracking) {
    if (!navigator.geolocation) return alert("GPS Mobile Supported nahi hai!");
    isTracking = true;
